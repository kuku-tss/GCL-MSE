...
from data import DrugDataLoader,NewDrugDataLoader,yvDrugDataLoader,gaiNewDrugDataLoader
from utils import *
...
def train(args, dataset, graph_data, cv):
    ...
    # opt_kl = torch.optim.Adam(model.parameters(), lr=0.0001)
    ...
    for iter_idx in range(1, args.train_max_iter):
        ...
        loss_drug = LOSS2(args, drug_out, drug_sim_out
                                            #   , batch_size=0, flag=0
                                              )
        loss_dis = LOSS2(args, dis_out, dis_sim_out
                                            #  , batch_size=0, flag=1
                                             )
...
# /home/liujin/miniconda3/envs/my_torch/bin/python /home/liujin/data/DRGCL-main/DRGCL-main/drug_train.py
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', default='/home/liujin/data/DRGCL-main/DRGCL-main/raw_data',type=str, help='The saving directory')
    parser.add_argument('--save_id', type=int, help='The saving log id')
    parser.add_argument('--data_name', default='twolrssl', type=str)#    'twoGdataset','twoCdataset','twolrssl'
    ...
    parser.add_argument('--beta', type=float, default=0.1)############################loss 0.1
    parser.add_argument('--num_neighbor', type=int, default=14)#######################################################################k值
    ...
    for times in range(1, 2):
        ...
        dataset = gaiNewDrugDataLoader(
            name=args.data_name,  # 选择数据集
            device=args.device,
            symm=args.gcn_agg_norm_symm,
            # base_k=2,  # 基础K值
            # k_strategy='learned',  # 动态K值策略
            k_min=2,  # 最小K值
            k_max=20,  # 最大K值
            # k_expand_factor=1.5,
            attention_epochs=300, # 注意力机制训练轮数
)
